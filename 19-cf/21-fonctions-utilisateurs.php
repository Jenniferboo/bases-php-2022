<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Les fonctions utilisateurs</title>
</head>
<body>
    <h1>Les fonctions utilisateurs</h1>
    <p>En PHP, on peut créer une nouvelle fonction en utilisant le mot clef 'function', suivi d'un nom (utilisation du même nommage que les variables sans le $) et de parenthéses qui peuvent contenir des arguments/paramètres</p>
    <pre><code>
function creeDesTensionsDansLeGrooupe(){
    return date("Y-m-d H:i:s");
}
    </code></pre>
    <?php
    //création de la fonction sans arguments
    function creeDesTensionsDansLeGrooupe(){
    //retour d'une valeur, ici le datetime
        return date("Y-m-d H:i:s");
    }
    echo creeDesTensionsDansLeGrooupe();
    // tout ce qui ce trouve après le return ne s'execute pas
    ?>
    <h2>les fonctons avec des arguments</h2>
    <p>Les paramètres ou arguments sont obligatoires si déclarés et sans valeurs par défaut</p>
    <pre><code>
        function calculBasic($num1,$num2,$op ="+"){

        }
    </code></pre>
    <?php
    function calculBasic($num1,$num2,$op ="+"){
        //on inverse la condition avec !, attention le && devient ||
        if (!is_numeric($num1)||!is_numeric($num2)){
            return "Attention, les 2 paramètres doivent être numériques";
        }
        switch ($op){
            case "*":
                return $num1*$num2;// le return arrête la fonction et renvoie le résultat
                break; //break facultatif dans ce cas particulier
                case "-":
                    return $num1-$num2;
                    break; 
                    case "/":
                        return $num1/$num2;
                        break; 
                        default :
                        return $num1+$num2;
        }
    }
    echo calculBasic(7, 5);

    ?>


</body>
</html>